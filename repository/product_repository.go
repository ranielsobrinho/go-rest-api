...
func (pr *ProductRepository) DeleteProductById(product_id int) error {
	...
	query.QueryRow(product_id)

	query.Close()
	return nil
}

func (pr *ProductRepository) UpdateProduct(product_id int, update_product model.Product) (int, error) {
	var id int
	...
	err = query.QueryRow(update_product.Name, update_product.Price, product_id).Scan(&id)
